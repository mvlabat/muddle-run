...
use bevy_egui::{egui, egui::Widget, EguiClipboard, EguiContext};
...
pub fn matchmaker_ui(
    ...
    loop {
        match main_menu_ui_channels.auth_message_rx.try_recv() {
            ...
            Ok(AuthMessage::DeviceCode {
                user_code,
                verification_url,
            }) => {
                log::debug!("Device code message");
                main_menu_ui_state.auth.screen = AuthUiScreen::Google;
                main_menu_ui_state.auth.user_code = user_code;
                main_menu_ui_state.auth.verification_url = verification_url;
                main_menu_ui_state.auth.pending_request = false;
                main_menu_ui_state.auth.password.clear();
                main_menu_ui_state.auth.error_message.clear();
            }
        ...
        AuthUiScreen::SignIn | AuthUiScreen::SignUp => {
            ...
            ui.horizontal(|ui| {
                    ...
                    .clicked()
                {
                    ...
                    auth_request_tx
                        .send(AuthRequest::RequestGoogleDeviceCode)
                        .expect("Failed to write to a channel (auth request)");
                    return;
                }
                ...
                |ui| {
                    egui::containers::Frame::none()
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.label(&auth_ui_state.user_code);
                        });
                    let button_label = if auth_ui_state.copied {
                        "Copied"
                    } else {
                        "Copy to clipboard"
                    };
                    if ui.button(button_label).clicked() {
                        auth_ui_state.copied = true;
                        copy_to_clipboard(egui_clipboard, &auth_ui_state.user_code);
                    }
                    ui.add_space(10.0);
                    ui.label("To sign in, follow the link below and paste the code:");
                    egui::widgets::Hyperlink::new(&auth_ui_state.verification_url).ui(ui);
                },
            );
        }
        AuthUiScreen::UnstoppableDomains => unimplemented!(),
    }
...
fn copy_to_clipboard(clipboard: &mut bevy_egui::EguiClipboard, text: &str) {
